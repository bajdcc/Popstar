    ...
    , m_sb(NEW_SB_SIZE, NEW_SB_COLORS)
{
    ...
    m_font.CreateFont(20, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
...
BOOL CPopstarDlg::OnInitDialog()
{
    ...
    if (GetCurrentDirectory(MAX_PATH, path_.GetBuffer(MAX_PATH)))
    {
        ...
        if (PathFileExists(path))
        {
            ...
            {
                m_iMaxColors = it;
                m_sb.SetColors(it, RGB(240, 240, 240));
                CString str;
                DWORD c;
                for (int i = 0; i < it; i++)
                {
...
void CPopstarDlg::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    else if ((nChar == 'r' || nChar == 'R') && !m_bEdit && StarBoard::m_pThread == NULL)
    {
        bool bT = true;
        m_sb.Terminate(bT);
        m_sb.Reset();
    ...
    else if ((nChar == 'c' || nChar == 'C') && !m_bEdit)
    {
        m_sb.RunWorkingThread();
    }
    ...
    else if ((nChar == 'p' || nChar == 'P') && StarBoard::m_pThread == NULL)
    {
    ...
    else if ((nChar == 'v' || nChar == 'V') && StarBoard::m_pThread == NULL)
    {
    ...
    else if ((nChar == 'e' || nChar == 'E') && StarBoard::m_pThread == NULL)
    {
        if (m_bEdit)
        {
    ...
    else if ((nChar == 'f' || nChar == 'F') && m_bEdit)
    {
        m_sb.FillColor(m_iChooseColor);
        Invalidate();
    }
    else if ((nChar == 't' || nChar == 'T') && StarBoard::m_pThread == NULL)
    {
...
void CPopstarDlg::OnTimer(UINT_PTR nIDEvent)
{
    ...
    if (nIDEvent == 0x40)
    {
        ...
        int lst;
        ULONG runover;
        LPCTSTR speed = StarBoard::m_bAccCalc ? TEXT("FAST") : TEXT("SLOW");
        m_sb.Lists(lst, false);
        m_sb.CurrentRunovers(runover, false);
        str.AppendFormat(_T("(%s) 队列数：%4d  结果数：%4d"), speed, lst, runover);
